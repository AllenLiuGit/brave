...
public class SpanCreationBenchmarks {
  ...
  @Benchmark
  public MutableSpan simpleRootSpan_brave4_span() {
    MutableSpan span = new MutableSpan(tracer.nextContext(null), recorder.localEndpoint);
    span.name("encode");
    span.start(recorder.epochMicros());
    span.finish(recorder.epochMicros(), null);
    return span;
  }

  @Benchmark
  public MutableSpan elaborateSpan_brave4_span() {
    MutableSpan span = new MutableSpan(tracer.nextContext(null), recorder.localEndpoint);
    span.kind(CLIENT);
    span.name("get");
    span.tag("clnt/finagle.version", "6.36.0");
    ...
    span.start(recorder.epochMicros());
    span.annotate(recorder.epochMicros(), Constants.WIRE_SEND);
    span.annotate(recorder.epochMicros(), Constants.WIRE_RECV);
    span.finish(recorder.epochMicros(), null);
